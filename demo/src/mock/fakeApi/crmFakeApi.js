...
export default function crmFakeApi (server, apiPrefix) {
    ...
    server.post(`${apiPrefix}/crm/patients`, (schema, {requestBody}) => {
    ...
    server.get(`${apiPrefix}/crm/patients-statistic`, () => {
        return {
            totalPatients: {
                value: 2420,
                growShrink: 17.2
    ...
    server.get(`${apiPrefix}/crm/patient-details`, (schema, {queryParams}) => {
        const id = queryParams.id
        const user = schema.db.userDetailData.find(id)
        return user
    })

    server.del(`${apiPrefix}/crm/patient/delete`, (schema, {requestBody}) => {
        const { id } = JSON.parse(requestBody)
        schema.db.userDetailData.remove({id})
        return {}
    })

    server.put(`${apiPrefix}/crm/patients`, (schema, {requestBody}) => {
